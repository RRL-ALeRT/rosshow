...
class TermGraphics(object):
    def __init__(self, mode=MODE_UNICODE, color_support=None):
        ...
        if self.color_support is None:
            if self.term_type in ["xterm-256color", "xterm"] or self.term_color in [
                "truecolor",
                "24bit",
            ]:
    ...
    def update_shape(self):
        ...
        """
        self.term_shape = tuple(
            reversed(
                list(map(lambda x: int(x), os.popen("stty size", "r").read().split()))
            )
        )
        ...
        if new_shape != self.shape:
            ...
            self.buffer = (
                np.frombuffer(
                    (b"\x28\x00" * (self.term_shape[0] * self.term_shape[1])),
                    dtype=np.uint16,
                )
                .reshape((self.term_shape[1], self.term_shape[0]))
                .copy()
            )
            self.colors = (
                np.frombuffer(
                    (b"\xff\xff\xff" * (self.term_shape[0] * self.term_shape[1])),
                    dtype=np.uint8,
                )
                .reshape((self.term_shape[1], self.term_shape[0], 3))
                .copy()
            )
            self.igrid, self.jgrid = np.meshgrid(
                np.arange(self.buffer.shape[1]), np.arange(self.buffer.shape[0])
            )
    ...
    def points(self, points, colors=None, clear_block=False):
        ...
        where_valid = (
            (i_array >= 0)
            & (j_array >= 0)
            & (i_array < self.term_shape[0])
            & (j_array < self.term_shape[1])
        )
    ...
    def image(
        self, data, width, height, point, image_type=IMAGE_MONOCHROME, clear_block=False
    ):
        """
        ...
        if image_type == IMAGE_MONOCHROME:
            ...
            screen_is, screen_js = np.meshgrid(
            ...
            where_valid = (
                (screen_is >= 0)
                & (screen_js >= 0)
                & (screen_is < self.shape[0])
                & (screen_js < self.shape[1])
                & (img > 0)
            )
            ...
            np.bitwise_and.at(self.colors, (screen_js >> 2, screen_is >> 1), 0)
            np.bitwise_or.at(
                self.colors,
            ...
            self.points(np.vstack((screen_is, screen_js)).T)
            np.bitwise_and.at(self.colors, (screen_js >> 2, screen_is >> 1), 0)
            np.bitwise_or.at(self.colors, (screen_js >> 2, screen_is >> 1), img)
            ...
            where_valid = (
                (screen_is >= 0)
                & (screen_js >= 0)
                & (screen_is < self.term_shape[0])
                & (screen_js < self.term_shape[1])
            )
            screen_is = screen_is[where_valid]
            screen_js = screen_js[where_valid]
            img = img[where_valid]
        ...
        for n in range(digrid.shape[0]):
            ...
            if np.any(c != current_draw_color):
                ...
                if self.color_support == COLOR_SUPPORT_24BIT:
                    sys.stdout.write(
                        "\033[38;2;{};{};{}m".format(
                            current_draw_color[0],
                            current_draw_color[1],
                            current_draw_color[2],
                        )
                    )
                elif (
                    self.color_support == COLOR_SUPPORT_256
                ):  # TODO support 256 colors but fall back to 16 for now
                    sys.stdout.write(
                        "\033[3" + str(self._rgb_to_8(current_draw_color)) + "m"
                    )
                elif (
                    self.color_support == COLOR_SUPPORT_16
                ):  # TODO actually implement colors 9-15
                    sys.stdout.write(
                        "\033[3" + str(self._rgb_to_8(current_draw_color)) + "m"
                    )
